...
class Ebook extends Model
{
    ...
    {
        return static::with(['files'])->isPrivate()->where('slug', $slug)->firstOrFail();
    }

    public function create_episode($ebook_id)
    {

        $ci_auth = Session::get('ci_auth');
        EbookHero::where('ci_auth',$ci_auth)
                ->whereNull('ebook_id')
                ->update([
            'ebook_id' => $ebook_id,
        ]);
        EbookEpisodes::where('ci_auth',$ci_auth)
        ->whereNull('ebook_id')
        ->update([
            'ebook_id' => $ebook_id,
        ]);
        EbookItemTemp::where('ci_auth',$ci_auth)
        ->whereNull('ebook_id')
        ->update([
            'ebook_id' => $ebook_id,
        ]);
        EbookSkillTemp::where('ci_auth',$ci_auth)
        ->whereNull('ebook_id')
        ->update([
            'ebook_id' => $ebook_id,
        ]);
        EbookCodewordTemp::where('ci_auth',$ci_auth)
        ->whereNull('ebook_id')
        ->update([
            'ebook_id' => $ebook_id,
        ]);
